...
export async function listTemplates({ project_path }) {
    const templates = new Map();
    // 1. 先加载内置模版
    const builtinFiles = await fs.readdir(BUILTIN_TEMPLATES_DIR).catch(() => []);
    for (const file of builtinFiles) {
        if (file.endsWith(".yaml")) {
            const content = await fs.readFile(path.join(BUILTIN_TEMPLATES_DIR, file), "utf-8");
    ...
    for (const file of userFiles) {
        if (file.endsWith(".yaml")) {
            const content = await fs.readFile(path.join(userTemplatesDir, file), "utf-8");
            const config = yaml.parse(content);
            templates.set(config.id, {
                id: config.id,
                name: config.name,
...
export async function loadTemplate({ template_id, project_path }) {
    // 1. 先尝试用户模版
    const userPath = path.join(project_path, USER_TEMPLATES_SUBDIR, `${template_id}.yaml`);
    try {
