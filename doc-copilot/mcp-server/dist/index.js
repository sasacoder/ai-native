...
var BUILTIN_TEMPLATES_DIR = process.env.TEMPLATES_DIR || "./templates";
var USER_TEMPLATES_SUBDIR = ".ai-native/doc-copilot/templates";
var listTemplatesSchema = external_exports.object({
  project_path: external_exports.string().describe("\u9879\u76EE\u76EE\u5F55\u8DEF\u5F84")
});
async function listTemplates({ project_path }) {
  const templates = /* @__PURE__ */ new Map();
  const builtinFiles = await fs.readdir(BUILTIN_TEMPLATES_DIR).catch(() => []);
  for (const file of builtinFiles) {
    if (file.endsWith(".yaml")) {
      const content = await fs.readFile(
        path.join(BUILTIN_TEMPLATES_DIR, file),
        "utf-8"
      );
  ...
  for (const file of userFiles) {
    if (file.endsWith(".yaml")) {
      const content = await fs.readFile(
        path.join(userTemplatesDir, file),
        "utf-8"
      );
      const config = yaml.parse(content);
      templates.set(config.id, {
...
var loadTemplateSchema = external_exports.object({
  template_id: external_exports.string().describe("\u6A21\u7248 ID"),
  project_path: external_exports.string().describe("\u9879\u76EE\u76EE\u5F55\u8DEF\u5F84")
});
async function loadTemplate({ template_id, project_path }) {
  const userPath = path.join(
    project_path,
    USER_TEMPLATES_SUBDIR,
    `${template_id}.yaml`
  );
  try {
    const content2 = await fs.readFile(userPath, "utf-8");
    return {
      content: [{
        type: "text",
        text: JSON.stringify(yaml.parse(content2), null, 2)
      }]
    };
  } catch {
  }
...
var STATE_DIR = process.env.STATE_DIR || ".ai-native/doc-copilot";
var STATE_FILE = process.env.STATE_FILE || "state.yaml";
var ChapterSchema = external_exports.object({
  name: external_exports.string(),
  status: external_exports.enum(["pending", "in_progress", "done"]),
  phase: external_exports.enum(["brainstorming", "outlining", "writing"]).optional(),
  outline_confirmed: external_exports.boolean().optional(),
...
var saveStateSchema = external_exports.object({
  project_path: external_exports.string().describe("\u9879\u76EE\u76EE\u5F55\u8DEF\u5F84"),
  state: StateSchema,
  render: external_exports.boolean().optional().describe("\u662F\u5426\u540C\u6B65\u6E32\u67D3\u6587\u6863")
});
async function saveState({ project_path, state, render }) {
  const stateDir = path2.join(project_path, STATE_DIR);
  const statePath = path2.join(stateDir, STATE_FILE);
  await fs2.mkdir(stateDir, { recursive: true });
  const content = yaml2.stringify({
    ...state,
    updated_at: (/* @__PURE__ */ new Date()).toISOString()
  });
  await fs2.writeFile(statePath, content);
  let outputPath = null;
  if (render) {
    outputPath = path2.join(project_path, state.output);
    await fs2.mkdir(path2.dirname(outputPath), { recursive: true });
    const doc = state.chapters.filter((ch) => ch.content).map((ch) => `## ${ch.name}

${ch.content}`).join("\n\n---\n\n");
    await fs2.writeFile(outputPath, doc);
  }
  return {
    content: [{
      type: "text",
...
var loadStateSchema = external_exports.object({
  project_path: external_exports.string().describe("\u9879\u76EE\u76EE\u5F55\u8DEF\u5F84")
});
async function loadState({ project_path }) {
  const statePath = path2.join(project_path, STATE_DIR, STATE_FILE);
  try {
    const content = await fs2.readFile(statePath, "utf-8");
    return {
      content: [{
        type: "text",
        text: JSON.stringify({ exists: true, state: yaml2.parse(content) }, null, 2)
      }]
    };
  } catch {
    return {
